...
function validateBoundedNumber(value, name, min, max) {
  const parsed = Number(value);
...
const name = validateText(valueFor("name", "我的 Codex QQ Skin"), "name", 80, "我的 Codex QQ Skin");
...
const layoutMode = validateChoice(
  valueFor("layout-mode", "classic-three-pane"),
  "layout-mode",
  ["classic-three-pane", "off"],
);
const rightPanel = validateChoice(valueFor("right-panel", "open"), "right-panel", ["open", "remember"]);
const layoutMinWidth = validateBoundedNumber(valueFor("layout-min-width", "1180"), "layout-min-width", 1080, 2400);
const rightPanelWidth = validateBoundedNumber(valueFor("right-panel-width", "300"), "right-panel-width", 272, 360);
...
const custom = {
  ...
  id: `custom-${Date.now()}`,
  kind: "custom-native",
  name: name || "我的 Codex QQ Skin",
  brandSubtitle: "CODEX CUSTOM SKIN",
  tagline: tagline || "把喜欢的画面变成可交互的 Codex 工作台。",
  projectPrefix: "选择项目 · ",
  projectLabel: "◉  选择项目",
  statusText: "CUSTOM SKIN ONLINE",
